...
import com.sample.tests.framework.ui.controls.TableView;
...
public class BasicKDTSteps {
    ...
    public void verifyTableRowCount(String list, String countValue) throws Exception {
        ...
        String expectedValue = countValue;
        for (String key : Context.variables()) {
            expectedValue = expectedValue.replaceAll(key, Context.get(key).toString());
        }
        Expression expression = new Expression(expectedValue);
    ...
    public void fieldValueIsCalculatedByFormula(String field, String formula) throws Exception {
        final double precision = 0.0099;
        double pageVal = Double.parseDouble(Page.getCurrent().onPage(field).getText());
        for (String key : Context.variables()) {
            formula = formula.replaceAll(key, Context.get(key).toString());
        }
        Expression expression = new Expression(formula);
        double calcVal = expression.setRoundingMode(RoundingMode.HALF_EVEN).setPrecision(6).eval().doubleValue();
        Assert.assertEquals("Wrong " + field + "! on page (" + pageVal + ") vs calculated (" + calcVal + ")",
                pageVal, calcVal, precision);
    }
}
